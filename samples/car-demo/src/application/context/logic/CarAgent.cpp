...
void
CarAgent::update(float step, const Circuit& circuit, const NeuralNetwork& in_NN) {
  if (std::isnan(_position.x) || std::isnan(_position.y) || std::isnan(_angle))
    _alive = false;
...
CarAgent::updateSensors() {
  ...
  static const std::array<float, 5> angles = {
    {-gero::math::pi / 4.0f, -gero::math::pi / 8.0f, 0.0f, gero::math::pi / 8.0f, gero::math::pi / 4.0f}};
...
void
CarAgent::collideSensors(const Lines& walls) {
  for (t_sensor& sensor : _sensors) {
    sensor.value = 1.0f;
...
void
CarAgent::collideCheckpoints(const Lines& checkpoints) {
...
void
CarAgent::collideWalls(const Lines& walls) {
